...
export default {
  ...
  methods: {
    getHeight(elementSelector, distance, wrapSelectorContent) {
      ...
      if (typeof distance === 'string' && Number.isNaN(+distance)) {
        const oF = document.querySelector(distance);
        if (oF) {
          _distance = oF.offsetHeight;
        }
      }
      if (wrapSelectorContent) {
        const oWrapSelectorContent = document.querySelector(wrapSelectorContent);
        if (oWrapSelectorContent) return oWrapSelectorContent.offsetHeight - _distance;
      }
