  ...
  <CommonDialogComp
    ...
    <div class="dialog-content">
      <el-table v-if="result" :data="result.rows" stripe border style="width: 100%;" height="240">
        ...
        <el-table-column prop="content" label="操作备注" show-overflow-tooltip width="320">
          <template #default="scope">
            <span style="color: #989898;">{{ scope.row.content }}</span>
          </template>
        </el-table-column>
...
import { IEquipmentHistory, TEquipment } from '../../types';
...
const result = ref<null | { rows: IEquipmentHistory[], count: number }>(null);
const getHistory = async (offset = 1) => {
  ...
  const resp = await mpEquipmentApi.getEquipmentHistoryList(condition.value).catch(() => null);
