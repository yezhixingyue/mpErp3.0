...
import { FormulaPanelElementSelectDialog, ResultFormulaTableComp } from '@/components/common/mpzj-sell-lib/lib';
...
export default {
  ...
  computed: {
    ...
    ...mapState('priceManage', ['curSolutionItem', 'curEditPriceItemData', 'curPriceItem', 'curCraftPriceItemData',
      'PriceItemAxisPropertyList', 'ResultFormulaList', 'PriceItemPropertyList']),
  ...
  watch: {
    PriceTableData: {
      handler(val, oldVal) {
        if (val && val.ID && (!oldVal || oldVal.ID !== val.ID) && this.initFetchResultFormula) {
          this.getFormulaList();
        }
      },
      immediate: true,
