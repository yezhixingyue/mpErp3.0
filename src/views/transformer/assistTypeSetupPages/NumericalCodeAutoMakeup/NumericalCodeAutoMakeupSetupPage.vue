...
const crumbsList = computed(() => {
  ...
  return [
    ...
    { name: '数码自动拼版', path: '/numericalCodeAutoMakeupList' },
    { name: `${GeneralMapDataClassData.value?.curEditItem ? '编辑' : '添加'}数码自动拼版条件：${TransformerListPageData.value?.curEditProductName}   ${partName}` },
  ];
});
...
<script lang='ts'>
export default {
  name: 'NumericalCodeAutoMakeupSetupPage',
};
</script>
