<template>
  <section class="wrap">
    <header>
      ...
      <div>
        <mp-button type="primary" size="small" @click="onMapClick(null)">+ 添加自动拼版条件</mp-button>
        <span class="warning ft-12 tips-box ml-47">
          <i class="el-icon-warning ft-14 mr-8 ml-5"></i>
          注：如果使用数码生产线，且匹配列表中任意一条，则
          <em class="is-bold ft-13">自动拼版</em>
        </span>
      </div>
    </header>
    <main class="page-main">
      ...
      <div v-else-if="GeneralMapDataClassData && !GeneralMapDataClassData.loading" class="empty-box">
        <mp-empty description="暂无自动拼版条件"></mp-empty>
      </div>
    </main>
    <footer>
...
import { NumericalCodeAutoMakeupLimitItemClass } from './NumericalCodeAutoMakeupLimitItemClass';
...
const crumbsList = computed(() => {
  ...
  return [
    ...
    { name: `数码自动拼版：${TransformerListPageData.value?.curEditProductName}   ${partName}` },
  ];
});
...
const onMapClick = (item: GeneralMapItemClass | null) => {
  ...
  router.push('/numericalCodeAutoMakeupSetup');
};
...
transformerStore.setGeneralMapDataClassData(new NumericalCodeAutoMakeupLimitItemClass(TransformerListPageData.value));

</script>

<script lang='ts'>
export default {
  name: 'NumericalCodeAutoMakeupListPage',
};
</script>
