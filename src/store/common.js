...
export default {
  ...
  actions: {
    ...
    async getProductClassifyData({ state, commit }, { bool = false, key = 6 } = {}) { // 获取产品二级分类数据1  -   2为客户 6为代客下单   ---- !
    ...
    /* 获取生产工厂列表数据
    -------------------------------*/
    async getFactoryList({ state, commit }, { isForce, includeConvertServer } = { isForce: false, includeConvertServer: false }) {
      // isForce为true时 代表必须从服务器重新获取数据  isForce:强制获取
      if (state.factoryList.length > 0 && !isForce) return;
      let list;
      let key = true;
      const resp = await api.getFactoryList(includeConvertServer).catch(() => { key = false; });
    ...
    async getMaterialAllList({ commit }) { // 获取全部物料列表
      ...
      const resp = await api.getMaterialList(params).catch(() => {});
