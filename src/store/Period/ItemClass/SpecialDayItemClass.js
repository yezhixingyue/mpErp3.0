...
export default class SpecialDayItemClass {
  ...
  SpecialType = SpecialTypeEnums.delay.value // [{ Name: '延长工时', ID: 1 }, { Name: '快递停发', ID: 2 }]   number
  ...
  constructor(data) {
    if (data) {
      ...
      } else if (SpecialType === SpecialTypeEnums.stop.value) {
        if (EndTime) {
  ...
  static submitChecker(data) {
    ...
    if (SpecialType === SpecialTypeEnums.delay.value) { // 选中延长工时
    ...
    } else if (SpecialType === SpecialTypeEnums.stop.value) { // 快递停发
  ...
  static transform(data) {
    if (!data) return null;
    const temp = { ...data };
    if (data.SpecialType === SpecialTypeEnums.delay.value) { // StartTime, EndTime
      temp.StartTime = getDateString(data.DelayStartTime, true);
      temp.EndTime = getDateString(data.DelayEndTime, false, true);
    } else if (data.SpecialType === SpecialTypeEnums.stop.value) {
