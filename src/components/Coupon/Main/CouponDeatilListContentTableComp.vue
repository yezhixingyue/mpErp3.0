<template>
<el-table
    class="mp-coupon-detail-table-comp-wrap mp-common-table-styles"
    :max-height="h"
    :height="h"
    fit
    :data="couponUseListData"
    stripe
    border
    style="width: 100%"
  >
    ...
    <el-table-column min-width="160px" label="内部领取人">
      <template slot-scope="scope">
        <span class="is-font-size-14"
         >{{scope.row.Record && scope.row.Record.ReceiveUser.StaffName}}</span>
      </template>
    </el-table-column>
      ...
      <ul class="handle-menus" slot-scope="scope">
        <li>
          <span
           ...
           @click="handleLookUpBindUsers(scope.row)"
           class="is-list-btn">
        ...
        <li>
          <span
           ...
           @click="handleRelatedOrder(scope.row, scope.$index)"
...
export default {
  ...
  methods: {
    ...
    handleRelatedOrder(rowData) {
      ...
      if (!OrderID) {
        this.messageBox.failSingleError('出错啦!', '[ 未获取到订单信息 ]', null, null);
        return;
      }
      this.$utils.handleLoadingHOF(
