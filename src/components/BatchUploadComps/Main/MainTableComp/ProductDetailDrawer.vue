        ...
        <footer v-if="curDetailData">
          <ul>
            ...
            <li class="print-content">
              <label>印刷内容：</label>
              <span>{{curDetailData.result.Content || '无印刷内容'}}</span>
            </li>
          </ul>
        </footer>
      </section>
...
<script>
import DisplayItem from '@/components/order/DialogContent/OrderDetailComps/DisplayItem.vue';
...
export default {
  ...
  computed: {
    ...
    OutPlatNo() {
      return this.curDetailData.result.OutPlate && this.curDetailData.result.OutPlate.Second ? this.curDetailData.result.OutPlate.Second : '';
    },
  },
  methods: {
