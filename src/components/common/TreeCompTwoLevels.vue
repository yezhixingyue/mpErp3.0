<template>
  <section
   class="mp-common-comps-two-level-tree-comp-wrap"
  ...
  computed: {
    treeListHasDisabled() {
      ...
      _list.forEach(level1 => {
        ...
        level1.children.forEach(level2 => {
          ...
          if (this.shouldDisabledList.includes(level2[this.defaultNodeKey])) {
            if (Array.isArray(_curChangeList)) {
              if (!_curChangeList.includes(level2[this.defaultNodeKey])) {
          ...
          // });
          // if (level2.children.length === _num1) {
          //   // eslint-disable-next-line no-param-reassign
          //   level2.disabled = true;
          //   _num1 += 1;
          // }
    ...
    checkAllIndeterminate() {
      ...
      return this.defaultCheckedKeys.length > 0
       && this.defaultCheckedKeys.length < this.twoLevelKeys.length;
    },
    AllKeys() {
      const _arr = [];
    ...
    checkAllComputed: {
      get() {
        ...
        return this.defaultCheckedKeys.length === this.twoLevelKeys.length;
      },
      set(newVal) {
