<template>
  ...
  >
    ...
    <template v-else-if="(!list || list.length === 0) && hideConstant">
    ...
    <template v-else>
      <ul class="list mp-scroll-wrap">
        ...
        <li v-if="canEmpty">
          <p class="mp-common-title-wrap">无</p>
          <div>
            <span class="blue-span" @click="onClick(null)">无</span>
          </div>
        </li>
        <li v-if="hideConstant && !canEmpty && list.length === 0" class="is-gray ft-12 pt-20">
          暂无数据
        </li>
      </ul>
    </template>
