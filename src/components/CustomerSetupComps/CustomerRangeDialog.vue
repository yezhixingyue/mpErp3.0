...
export default {
  ...
  computed: {
    title() {
      return this.itemData && (this.itemData.CategoryID || this.itemData.CategoryID === 0) ? '编辑客户等级' : '添加客户等级';
    },
  },
  methods: {
    onCancle() { // 取消  关闭弹窗
    ...
    checker() {
      ...
      if (!this.$utils.getValueIsOrNotNumber(this.localItemData.Value, true)) {
        this.messageBox.failSingleError('保存失败', '信誉额度必须为整数数字类型');
        return false;
      }
      if (this.localItemData.Value < 0) {
        this.messageBox.failSingleError('保存失败', '信誉额度不能为负');
        return false;
      }
