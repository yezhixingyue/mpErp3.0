  ...
  >
    <ul v-if="localItemData" class="content">
      ...
      <li>
        ...
        <div>
          <el-radio-group v-model="localItemData.Value">
            <el-radio
              v-for="it in CustomerPriceTypeEnumList"
              :label="it.ID"
              :key="it.ID"
              >
              {{it.Name}}
...
export default {
  ...
  data() {
    return {
      localItemData: null,
      CustomerPriceTypeEnumList,
    };
  },
  computed: {
    title() {
      return this.itemData && (this.itemData.CategoryID || this.itemData.CategoryID === 0) ? '编辑客户分类' : '添加客户分类';
    },
    CustomerPriceTypeEnumIDs() {
      return this.CustomerPriceTypeEnumList.map(it => it.ID);
    },
  },
  methods: {
    onCancle() { // 取消  关闭弹窗
    ...
    checker() {
      ...
      if (!this.CustomerPriceTypeEnumIDs.includes(this.localItemData.Value)) {
        this.messageBox.failSingleError('保存失败', '请选择定价方式');
        return false;
      }
